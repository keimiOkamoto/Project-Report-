...
As discussed in the previous chapter, the primary draw back of existing systems such as the smart fridge and the mobile applications is the lack of automation in the item registration process. Convincing consumers to alter habitual behaviour such as restocking the fridge is challenging and would most likely be met with resistance unless the newer process is to yields better results than the current. Thus the user having to either manually punch in the product name, or scan every item with a barcode reader can discourage the user from utilising the technology. No matter how accurate the monitoring technology is or how efficiently the data may be retrieved, if the loading of the fridge requires more time and effort than the previous process then the system will most likely not be used.   

\subsubsection{Current Item Level Product Identification Methods} 
Predominantly, the current methods of item level identification in the supply chain utilises the 'picket-fence' style, one-dimensional barcodes. add image Barcodes have been prevalent in grocery stores since the 1970's as a means to manage inventory and to accelerate the checkout process. A laser scans the code and the catalogued corresponding code in the database is used to retrieve detailed data such as the item name and cost of the product. But the code is limited to identifying a collection of items rather than the individual item level, omitting crucial individual information such as sell-by-date and fine-grain information of the origin of the produce.  The scanning of the barcode requires that nothing obstructs the line of sight between the code and the laser,  even the smallest scratch or dirt can render the code unreadable. Simultaneous reads are not possible the size of the code is limited meaning very little information can be held.
...
Why i chose android over iOS, web frame uber
\subsubsection{Java}
